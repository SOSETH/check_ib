...
IBHostValidator::IBHostValidator(const std::string& hostname) : IBValidator(), myHostName(hostname) {}
IBHostValidator::IBHostValidator(const std::string&& hostname) : IBValidator(), myHostName(hostname) {}

bool IBHostValidator::isValid(std::shared_ptr<IBNetfileParser> parser, std::shared_ptr<IcingaOutput> output, std::shared_ptr<IBHostRegistry> registry) throw(IBException) {
    bool isOK=true;
